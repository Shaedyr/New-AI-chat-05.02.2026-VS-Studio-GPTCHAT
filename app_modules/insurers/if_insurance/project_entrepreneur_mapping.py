...
def _extract_reklamasjon_amount(pdf_text: str) -> str:
    norm = _normalize_text(pdf_text)
    patterns = [
        r"Skade\s+i\s+reklamasjonstiden\s+pga\.?\s+mangel\s+i\s+entreprisen\s*([0-9][0-9\s.,]{2,})",
        r"reklamasjonstiden.*?mangel\s+i\s+entreprisen\s*([0-9][0-9\s.,]{2,})",
    ]
    for pattern in patterns:
        match = re.search(pattern, norm, flags=re.IGNORECASE | re.DOTALL)
        if match:
...
def _transform_if(pdf_text: str) -> dict:
    ...
    allrisk_raw = _extract_allrisk_amount(pdf_text)
    allrisk_digits = _digits(allrisk_raw)
    if allrisk_digits:
        out["A3"] = "Allrisk"
        out["B3"] = int(allrisk_digits)

    reklamasjon_raw = _extract_reklamasjon_amount(pdf_text)
    reklamasjon_digits = _digits(reklamasjon_raw)
    if reklamasjon_digits:
        out["A4"] = "Skade i reklamasjonstiden pga. mangel i entreprisen"
        out["B4"] = int(reklamasjon_digits)
...
def transform_data(extracted: dict) -> dict:
    """
    Dynamic mapping for the sheet "Prosjekt,entreprenor" (IF).
    """
