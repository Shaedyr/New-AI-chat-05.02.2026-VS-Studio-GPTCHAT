# app_modules/insurers/frende/vehicle_mapping.py
"""
FORDON mapping (frende).
Hard-isolated: this module only runs frende extractor logic.
"""

import re
import streamlit as st
from app_modules.Sheets.Fordon.extractors.frende import extract_frende_vehicles
...
def transform_data(extracted: dict) -> dict:
    ...
    st.write("?? FORDON: frende isolated mapping")

    try:
        vehicles = extract_frende_vehicles(pdf_text) or []
    except Exception as exc:
        st.error(f"frende extractor failed: {exc}")
        return out
