...
from app_modules.insurers.landkreditt.sheet_config import SHEET_MAPPINGS, transform_for_sheet
...
def _should_skip_manual_cell(sheet_name: str, cell_ref: str) -> bool:
    """
    Manual-entry policy:
    - In 'Alminnelig ansvar', column G is always manual.
      The program must never write values there.
    """
    if (sheet_name or "").strip().lower() == "alminnelig ansvar":
        return str(cell_ref).upper().startswith("G")
    return False
...
def _fill_static_sheet(ws, cell_map: dict, transformed_data: dict) -> int:
    ...
    for field_key, cell_ref in cell_map.items():
        if _should_skip_manual_cell(sheet_name, cell_ref):
            continue
        cell = ws[cell_ref]
