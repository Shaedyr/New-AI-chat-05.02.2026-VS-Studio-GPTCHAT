# app_modules/insurers/landkreditt/vehicle_mapping.py
"""
FORDON mapping (landkreditt).
Hard-isolated: this module only runs landkreditt extractor logic.
"""

import re
import streamlit as st
from app_modules.Sheets.Fordon.extractors.landkreditt import extract_landkreditt_vehicles
...
def transform_data(extracted: dict) -> dict:
    ...
    st.write("?? FORDON: landkreditt isolated mapping")

    try:
        vehicles = extract_landkreditt_vehicles(pdf_text) or []
    except Exception as exc:
        st.error(f"landkreditt extractor failed: {exc}")
        return out
